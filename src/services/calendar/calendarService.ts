...
export class CalendarService {
  ...
  public async getDefaultCalendar(): Promise<Calendar> {
    await this.simulateNetworkDelay();
    const defaultCalendar = this.calendars.find(cal => cal.isDefault);
    if (!defaultCalendar) {
  ...
  public async deleteCalendar(id: string): Promise<boolean> {
    ...
    if (!calendarToDelete || calendarToDelete.isDefault) {
      return false;
    }
    
    // Delete all events in this calendar
    this.events = this.events.filter(event => event.calendar.id !== id);
  ...
  public async getEvents(filter?: CalendarFilter): Promise<CalendarEvent[]> {
    ...
    if (filter) {
      ...
      filteredEvents = filteredEvents.filter(event => 
          filter.calendarIds!.includes(event.calendar.id)
        );
      }
